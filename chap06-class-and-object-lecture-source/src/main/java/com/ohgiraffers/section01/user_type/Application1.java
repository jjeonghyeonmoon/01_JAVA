...
public class Application1 {
    ...
    String[] hobbies = new String[] {"야구", "코딩", "누워있기"};
        ...
        *  */
        ...
        /* comment.
        *   클래스 변수명 = new 클래스명();
        *   - 위에 구문을 객체(instance) 를 만드는 구문이다.
        *   - ex) int[] arr = new int[5];
        *   클래스는 사용자 정의의 자료형 되며, 클래스가 가지고 있는
        ...
        * */
        ...
        /* index. 3. 생성된 인스턴스(객체)의 초기값 확인하기*/
        /*comment.
        *  우리가 Member 클래스에 작성한 전역벽수(필드)에 접근하기 위해서는
        *   레퍼런스변수명.변수명 <- 이 방법으로 접근 할 수 있다.
        *  '.'  은 참조연산자 라고 하는데, 래퍼런스 변수가 참조하고 있는(주소를 가르키고)
        *   곳으로 접근한다 라는 의미이다.*/
        ...
        /* index. 2-2. 필드(전역변수)에 접근해서 변수처럼 사용해보기 */
        member.id = "user02";
        member.pwd = "pass02";
        member.name = "조평순";
        member.age =20;
        ...
        System.out.println("변경 후 member.hobbies : " + member.hobbies);
        for (int i = 0; i < member.hobbies.length; i++) {
            System.out.println(member.hobbies[i] + " ");
        }
