...
public class FinalFieldTest {
        ...
        *   final 키워드는 사용하는 위치가 다양하지만, 결국 별경 불가의 의미를 갖는다.
        ...
        *   초기화 이후 값을 변경할 수 없기 떄문에 바로 초기화를 시켜주어야 한다.
...
//        private final int nonstaticNum;
    ...
    //    private static final int STATIC_NUM; // 0 이라는 기본값이기 때문에 오류발생
      private static final double PI = 3.14;

      /* comment.
      *     static 이 붙은 키워드는 생성자를 이용한 초기화가 불가능하다.
      *     생성자는 인스턴스가 생성되는 시점(new 를 만나는 시점)에 호출이 되기 떄문에
      *     그 전에는 초기화가 일어나지 못한다.
      *     하지만 static 키워드는 프로그램이 run 하는 동시에 할당 되기 때문에
      *     초기화가 되지 않은 상태로 선언 된 것과 동일하여
      *     기본값을 초기화 된 후 값을 변경하지 못하기 때문에 에러가 발생한다.*/
